...
async function downloadPDF(content, filename) {
  ...
  const mL = 10; const mR = 10; const mT = 14; const mB = 14;
  const pageW = doc.internal.pageSize.getWidth();
  const pageH = doc.internal.pageSize.getHeight();
  const cW = pageW - mL - mR;
  const BULLET_INDENT = 4; const BULLET_HANG = 9; // consistent bullet grid
  const ROLE_LINE_H = 6; const BODY_LINE_H = 5.4;
  let y = 0;
  ...
  if (parsed.name || parsed.headerLines.length > 0) {
    ...
    doc.roundedRect(mL, boxTop, pageW - mL - mR, headerH, 3, 3, 'F');
    ...
    y = boxTop + headerH + 6; // gap below header before first section
  } else {
    y = mT;
  }
  ...
  for (let si = 0; si < parsed.sections.length; si++) {
    ...
    // Section label — small rounded pill, sized to the text (not a full-width bar)
    doc.setFont('helvetica', 'bold'); doc.setFontSize(9);
    const sf0 = doc.internal.scaleFactor;
    const labelText = sec.title.toUpperCase();
    const labelTextW = doc.getStringUnitWidth(labelText) * 9 / sf0;
    const pillPadX = 3.2; const pillH = 6.2;
    const pillW = labelTextW + pillPadX * 2;
    doc.setFillColor(...ACCENT_BG);
    doc.roundedRect(mL, y - pillH + 1.8, pillW, pillH, pillH / 2, pillH / 2, 'F');
    ...
    for (let ii = 0; ii < items.length; ii++) {
      ...
      if (item.type === 'roleHeader') {
        ...
        if (isBodySec || (!company && !date && role.length > 60)) {
          if (isSkillsSec) {
            // Compact horizontal skill list with purple pipe separators
            const skills = item.text.split(/\s*[,|•·]\s*/).map(s => s.trim()).filter(Boolean);
            doc.setFontSize(9.5);
            const sf = doc.internal.scaleFactor;
            const pipe = '   |   ';
            const pipeW = doc.getStringUnitWidth(pipe) * 9.5 / sf;
            let lineX = mL; chk(BODY_LINE_H);
            skills.forEach((s, si2) => {
              doc.setFont('helvetica', 'normal'); doc.setTextColor(...BODY);
              const w = doc.getStringUnitWidth(s) * 9.5 / sf;
              if (lineX + w > mL + cW && lineX > mL) { y += BODY_LINE_H; chk(BODY_LINE_H); lineX = mL; }
              doc.text(s, lineX, y); lineX += w;
              if (si2 < skills.length - 1) { doc.setTextColor(...ACCENT); doc.text(pipe, lineX, y); lineX += pipeW; }
            });
            y += BODY_LINE_H;
            continue;
          }
      ...
      if (item.type === 'bullet') {
        ...
        const textX = mL + BULLET_HANG;
        const textW = cW - BULLET_HANG;
        doc.setFont('helvetica', 'normal'); doc.setFontSize(9.5); doc.setTextColor(...BODY);
        const wrapped = doc.splitTextToSize(item.text, textW);
        chk(BODY_LINE_H);
        doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(...DARK_GRAY);
...
async function downloadDOCX(content, filename) {
  ...
  if (parsed.name || parsed.headerLines.length > 0) {
    ...
    if (parsed.name) {
      headerChildren.push(new Paragraph({
        ...
        children: [new TextRun({ text: parsed.name.toUpperCase(), bold: true, size: 32, font: CAL, color: DARK_GRAY })],
        ...sp(120, 60),
      }));
    }
  ...
  for (let si = 0; si < parsed.sections.length; si++) {
    ...
    // Section label — small rounded-look pill (single-cell shaded table, sized
    // to content rather than a full-width bar), no icon
    paragraphs.push(new Table({
      ...
      rows: [new TableRow({
        children: [new TableCell({
          ...
          children: [new Paragraph({
            children: [new TextRun({ text: sec.title, bold: true, size: 16, font: CAL, color: ACCENT, allCaps: true })],
          })],
        })],
      })],
      ...sp(si === 0 ? 40 : 140, 100),
      ...
      if (item.type === 'roleHeader') {
        ...
        if (isBodySec || (!company && !date && role.length > 60)) {
          if (isSkillsSec) {
            ...
            const runs = [];
            skills.forEach((s, si2) => {
              runs.push(new TextRun({ text: s, size: 19, font: CAL, color: BODY_CLR }));
              if (si2 < skills.length - 1) runs.push(new TextRun({ text: '   |   ', size: 19, font: CAL, color: ACCENT }));
            });
            ...
            rows: [new TableRow({
              children: [
                new TableCell({
                  ...
                  borders: allNoBorder,
                  children: [new Paragraph({ children: [new TextRun({ text: role, bold: true, size: 22, font: CAL, color: DARK_GRAY })], ...sp(100, 10) })],
                }),
                new TableCell({
...
// Official brand marks, required as-is (not recolored/restyled) by each
// provider's sign-in button guidelines. Inline SVG, no icon-library
// dependency added -- matches this file's existing inline-<svg> pattern.
function GoogleIcon() {
  return (
...
function AppleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
...
function AuthPage({ t, authLinkErrorCode, setPage }) {
  ...
  // Same shape as handleGoogle -- signInWithOAuth redirects the browser away
  // to the provider, then back to this same origin's PKCE callback handling
  // in useAuth (isAuthCallbackUrl/syncFromSession), completely provider-agnostic.
          ...
          {forgotPassword ? (
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 20 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0", color: C.textMuted, fontSize: 12, fontWeight: 600 }}>
                  <div style={{ flex: 1, height: 1, background: C.border }} />
                  {t("auth.orDivider")}
                  <div style={{ flex: 1, height: 1, background: C.border }} />
                </div>
                <Btn variant="secondary" onClick={handleGoogle} disabled={loading} style={{ width: "100%", justifyContent: "center", padding: "12px 22px" }}>
                  <GoogleIcon /> {t("auth.continueWithGoogle")}
                </Btn>
                <Btn variant="secondary" onClick={handleApple} disabled={loading} style={{ width: "100%", justifyContent: "center", padding: "12px 22px" }}>
...
Write a complete, polished ATS-friendly resume in plain text. Include: Contact Information, Professional Summary, Work Experience (with bullet points and quantified achievements where possible), Skills, Education${aiForm.certifications ? ", Certifications" : ""}. Use UPPERCASE for section headers. Use action verbs. Return ONLY the resume text — no explanation, no markdown, no preamble.`;
  ...
  const applyWeakBulletFix = (original, improved) => {
    ...
    // Marks the working text as edited, same flag handleImproveResume already sets after
    // its own edits -- without it, handleSaveToLibrary treats `resume` as still the
    // untouched original and silently skips saving this edit at all.
    setIsOptimized(true);
  };
...
function TrackerPage({ applications, deleteApplication, saveApplication, resumes, savedJobs, smartApplyQueue, profile, isPremium, outcomePatternsHook, outcomeAnalysesHook, recommendationEvalHook, forceInsightsTab, onForceInsightsTabHandled, loadError }) {
  const { t } = useI18n();
  ...
  return (
    <div>
      {loadError && <div style={{ background: "#FEF2F2", border: "1.5px solid #FCA5A5", borderRadius: 8, padding: "10px 14px", marginBottom: 12, color: "#DC2626", fontSize: 13 }}>{t("tracker.loadError")}</div>}
...
function ProfilePage({ profile, updateProfile, onOnboardingSave }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setSaving(false);
      return; // DB write failed -- don't show success or advance onboarding
    }
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {saved && <div style={{ background: C.greenLight, border: `1px solid ${C.green}30`, borderRadius: 9, padding: 12, color: C.green, fontSize: 13, marginBottom: 14 }}>{t("profile.savedSuccess")}</div>}
        <Btn onClick={save} disabled={saving} loading={saving} style={{ padding: "12px 28px" }}>{saved ? t("profile.saved") : t("profile.saveChanges")}</Btn>
      </Card>
    </div>
  );
}
...
export default function App() {
  ...
  const updateProfile = async (updates) => {
    ...
    saveAccount(updated);
    // Awaited (not fire-and-forget) so a failed write reaches the caller as a
    // rejected promise instead of being silently discarded -- callers that
    // don't await this (most call sites) are unaffected: the local/optimistic
    // update above still happens exactly as before, only a DB failure now
    // surfaces to whoever does await it (e.g. ProfilePage's save()).
    if (updated.id) await upsertProfile(updated.id, updates);
  };
