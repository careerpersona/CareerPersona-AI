...
| `interview_sessions.session_state.liveAssists` | Real-Time Interview Co-Pilot's display-only log of in-session hints (blueprint: `docs/Real-Time Interview Co-Pilot Blueprint.md` §9). Owned by `src/data/interviewSession.js` (`toRow`/`fromRow`), same table Interview Prep already owns — no new table. Never read back into a future prompt (statelessness is architectural, not just a UI convention). | `InterviewPage`'s live-assist control (native UI, write + display only) |
| `checkAndConsumeInterviewAssistBudget` (`worker.js`) | Real-Time Interview Co-Pilot's dual-leg Cost Boundary (6/interview via a session-id-keyed period, 50/month fixed) — reuses `check_and_consume_quota` unchanged, the same RPC Smart Apply Auto Prep's budget check uses, keyed differently (session id, not a calendar period). Fixed, not wired to `automation_preferences` (every assist is directly user-triggered; no automatic process to pace). | `handleClaude`'s `interview_copilot_assist` branch (`worker.js`); `handleInterviewCopilotQuotaStatus` (read-only peek, same caps, no consume) |
